...
function buildSystemPrompt(APP_KNOWLEDGE_BASE) {
    return `আপনি একজন অভিজ্ঞ স্বপ্ন বিশ্লেষক। আপনার নাম "Dream Lans", নির্মাতা স্বাধীন ডেভেলপার "রাহুল দেব"।
...
৪. ব্যক্তিগতকরণ: ইউজারের পেশা/বয়স/সম্পর্ক/পরিস্থিতি জানা থাকলে (এই বার্তায় বা "[ইউজারের জীবন-প্রেক্ষাপট জানা আছে...]" নোটে) তা দিয়ে ব্যাখ্যাকে বাস্তব জীবনের সাথে নির্দিষ্টভাবে সংযুক্ত করুন — যেমন ছাত্র হলে পড়াশোনা/পরীক্ষার চাপ প্রসঙ্গে, কৃষক হলে জমি/ফসল/আবহাওয়া প্রসঙ্গে, চাকরিজীবী হলে কর্মক্ষেত্র/বসের চাপ প্রসঙ্গে ব্যাখ্যা করুন। প্রেক্ষাপট না থাকলে সাধারণ ব্যাখ্যা দিন, অনুমান করে বানাবেন না।
...
// dream টেক্সটে স্থান/অনুভূতি/পেশার ইঙ্গিত আছে কিনা তা keyword দিয়ে যাচাই করা হয় (deterministic check)
const LOCATION_HINTS = ['বাসা', 'বাড়ি', 'ঘর', 'রাস্তা', 'অফিস', 'স্কুল', 'কলেজ', 'মাঠ', 'জঙ্গল', 'বন', 'পানি', 'নদী', 'তীর', 'তির', 'পুকুর', 'লেক', 'সাগর', 'সমুদ্র', 'দ্বীপ', 'বাইরে', 'গ্রাম', 'শহর', 'ছাদ', 'বাজার', 'হাসপাতাল', 'মন্দির', 'মসজিদ', 'পাহাড়', 'আকাশ', 'ট্রেন', 'বাস', 'স্টেশন', 'দোকান', 'পার্ক', 'বিয়েবাড়ি', 'অনুষ্ঠান'];
...
// পেশা/জীবন-প্রেক্ষাপট — এটা প্রতি স্বপ্নে না, বরং পুরো কথোপকথন জুড়ে একবার জানলেই যথেষ্ট (নিচে আলাদাভাবে হ্যান্ডল হয়)
...
// পুরো কথোপকথন (বর্তমান বার্তা + history) জুড়ে পেশা/জীবন-প্রেক্ষাপট আগে কখনো বলা হয়েছে কিনা খুঁজে বের করে
...
export default async function handler(req, res) {
    ...
    // ── Dynamic import: এখানে করা হচ্ছে (ফাইলের উপরে static import না) —
    // যাতে info.js বা symbols/index.js লোড করতে কোনো সমস্যা হলেও তা try/catch-এ ধরা পড়ে
    // এবং debug error হিসেবে সরাসরি চ্যাটে দেখা যায়, silent crash না হয়ে
    const { APP_KNOWLEDGE_BASE } = await import('./info.js');
    const { findMatchedSymbols, buildContextBlock } = await import('./symbols/index.js');
    ...
    // পুরো কথোপকথন জুড়ে পেশা/জীবন-প্রেক্ষাপট আগে জানা আছে কিনা চেক করা হচ্ছে
    const knownProfession = findKnownProfession(dreamTrimmed, history);
    const professionNote = knownProfession
        ? `\n[ইউজারের জীবন-প্রেক্ষাপট জানা আছে: "${knownProfession}" — এই প্রেক্ষাপট অনুযায়ী ব্যাখ্যাটি প্রাসঙ্গিক করে তুলুন (যেমন ছাত্র হলে পড়াশোনা/পরীক্ষা প্রসঙ্গে, কৃষক হলে জমি/ফসল প্রসঙ্গে, চাকরিজীবী হলে কর্মক্ষেত্র প্রসঙ্গে ইত্যাদি)। এই তথ্য সম্পর্কে আবার জিজ্ঞেস করবেন না।]`
        : '';
    ...
    if (inputType === 'context_answer') {
        ...
        // প্রথমবার প্রশ্ন করার সুযোগ থাকলেই (contextQuestionAlreadyAsked===false) শুধু পেশা অনুপস্থিত তালিকায় যোগ হবে —
        // এরপর আর কখনো আলাদা করে পেশা নিয়ে প্রশ্ন repeat হবে না
        ...
        contextDirective = missing.length > 0
            ? `\n[সিস্টেম-নির্ধারিত ইনপুট-টাইপ: (ক) নতুন স্বপ্ন। এই স্বপ্নের বর্ণনায় নিচের তথ্য অনুপস্থিত: ${missing.join(' এবং ')}। "প্রাসঙ্গিক প্রশ্ন" অংশে এর মধ্যে সর্বোচ্চ ২টা গুরুত্বপূর্ণ তথ্য নিয়ে সংক্ষেপে জিজ্ঞেস করুন (সবগুলো একসাথে না, ভারী মনে হবে)। অন্য কোনো ধরনের প্রশ্ন করবেন না।]` + professionNote
            : '\n[সিস্টেম-নির্ধারিত ইনপুট-টাইপ: (ক) নতুন স্বপ্ন। এই স্বপ্নের বর্ণনায় প্রয়োজনীয় তথ্য উল্লেখ আছে। তাই "প্রাসঙ্গিক প্রশ্ন" অংশে কোনো তথ্য-সংগ্রহ প্রশ্ন করবেন না, শুধু একটা ছোট, সাধারণ কথোপকথনমূলক প্রশ্ন দিন।]' + professionNote;
    }
